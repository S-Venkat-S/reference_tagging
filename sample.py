...
id = "1"
xml_text = "May, C. A., & Narfstr�m, K. (2012). Retinal capillary morphology in the Abyssinian cat with hereditary retinal degeneration. Experimental Eye Research, 99, 45-47. doi:https://doi.org/10.1016/j.exer.2012.03.022"
...
if references_json:
    ...
    for i in tag_list:
        ...
        for j in ref:
            #Find author name
            if j==i=="author":
                per_grp = soup.new_tag(con["per_grp"])
                mix.append(per_grp)
                for k in ref["author"]:
                    if len(k)>1:
    ...
    # xml_output = str(soup).replace('<?xml version="1.0" encoding="utf-8"?>', '')    
